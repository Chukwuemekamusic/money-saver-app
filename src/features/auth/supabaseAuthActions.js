...
import { syncUserURL, getUserURL, logoutUserURL } from "../../api/axiosUtil";
  ...
  async (_, { rejectWithValue, dispatch }) => {
    try {
      ...
      if (hasOAuthTokens) {
        ...
        if (sessionError) {
          console.error(
            "🔴 Failed to set session from URL tokens:",
            sessionError
          );
          throw sessionError;
        }
        ...
        if (setSessionData.session) {
          ...
          const isNewUser =
            setSessionData.session.user.created_at ===
            setSessionData.session.user.last_sign_in_at;

          // Always sync user with FastAPI backend
          await dispatch(
            syncUserWithBackend(setSessionData.session.access_token)
          );
      ...
      if (authData.session) {
        ...
        const isNewUser =
          ...
          authData.session.user.last_sign_in_at;
  ...
  async (token, { rejectWithValue }) => {
    try {
      ...
      if (error.response?.status === 401) {
        console.error(
          "🔴 Authentication failed during sync - token might be invalid"
        );
      ...
      console.log(
        "🟠 Backend sync failed but continuing - user can still use Supabase auth"
      );
